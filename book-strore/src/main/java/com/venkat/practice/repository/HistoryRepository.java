...
public interface HistoryRepository extends JpaRepository<ContentHistory, Long> {
	...
	public List<Result> findStats1(long bookId, Pageable pageable);

	@Query("SELECT new com.venkat.practice.domain.Result(c.transactionId,c.transactionTime,SUM(c.wordCount) AS WORD_COUNT)"
			+ " FROM ContentHistory c WHERE c.chapter.book.bookId=?1 "
			+ " group by c.transactionId,c.transactionTime order by c.transactionId DESC")
